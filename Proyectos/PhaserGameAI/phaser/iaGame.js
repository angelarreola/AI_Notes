...
function recopilarDatosEntrenamiento() {
    if (!modoAuto) {
        // Asegúrate de que solo recopilas datos cuando las balas estén activas
        if (balaD) {
            datosEntrenamiento.push({
                'input': [desplazamientoBala, velocidadBala, 0, 0],
                'output': [estadoEnAire, estadoEnSuelo, estadoMovIzquierda, estadoMovDerecha] // Salidas para saltar o no saltar
            });
        }
        ...
        if (balaVerticalD) {
            datosEntrenamiento.push({
                ...
                'output': [estadoEnAire, estadoEnSuelo, estadoMovIzquierda, estadoMovDerecha] // Necesitas definir esto
            });
        }

        // console.log({desplazamientoBala, velocidadBala, estadoEnAire, estadoEnSuelo, estadoMovDerecha, estadoMovIzquierda});
...
function datosDeEntrenamiento(param_entrada) {
    ...
    console.log("Salida", `En el Aire %: ${salidaRedondeada[0]}, En el suelo %: ${salidaRedondeada[1]}, Mov Izquierda %: ${salidaRedondeada[2]}, Mov Derecha %: ${salidaRedondeada[3]}`);
...
function gestionarMovimientoJugador() {
    if (modoAuto) {
        ...
        if (nnSalida[2] > nnSalida[3] /*&& jugador.position.x > DISTANCIA_PARED*/) { // Si la red decide "moverse a la izquierda"
        ...
        else if (nnSalida[3] > nnSalida[2] && jugador.position.x < config.width - 400 /*&& jugador.position.x < config.width - 300*/) { // Si la red decide "moverse a la derecha"
            if (inversionMov) {
                moverseIzquierda();
            } else {
                if (jugador.position.x >= config.width - 400) {
        ...
        if (teclaIzquierda.isDown) {
            moverseIzquierda();
        } else if (teclaDerecha.isDown && jugador.position.x < config.width - 400) {
            moverseDerecha();
        } else {
            jugador.body.velocity.x = 0; // Quieto
        }
