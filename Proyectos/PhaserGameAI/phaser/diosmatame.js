...
function iniciarRedNeural() {
    ...
    // nnNetwork = new synaptic.Architect.Perceptron(4, 9, 9, 9, 4); //
...
function actualizarEstadoJugador() {
    ...
    if (estadoMovDerecha === 0) {
        estadoMovIzquierda = 1;
    }
    if (estadoMovIzquierda === 0) {
        estadoMovDerecha = 1;
    }
    console.log(estadoMovDerecha, estadoMovIzquierda);
}
...
function gestionarMovimientoJugador() {
    if (modoAuto) {
        ...
        if (nnSalida[2] > UMBRAL > nnSalida[3] /*&& jugador.position.x > DISTANCIA_PARED*/) { // Si la red decide "moverse a la izquierda"
        ...
        else if (nnSalida[3] > UMBRAL > nnSalida[2] && jugador.position.x < config.width - 400 /*&& jugador.position.x < config.width - 300*/) { // Si la red decide "moverse a la derecha"
            if (inversionMov) {
                moverseIzquierda();
            } else {
                if (jugador.position.x >= config.width - 400) {
