...
const config = {
  ...
  function iniciarRedNeural() {
    // nnNetwork = new synaptic.Architect.Perceptron(5, 16, 2); //? Funciona ok ? ya ni me acuerdo con que
    // nnNetwork = new synaptic.Architect.Perceptron(5, 12, 12, 3); //? Funciona con las entradas no separadas. !olol
    // nnNetwork = new synaptic.Architect.Perceptron(5, 7, 7, 7, 3); //? No
    // nnNetwork = new synaptic.Architect.Perceptron(5, 14, 14, 14, 3); //? No (Salta a lo wey)
    // nnNetwork = new synaptic.Architect.Perceptron(5, 16, 8, 4, 3); //? No XD
    // nnNetwork = new synaptic.Architect.Perceptron(5, 12, 3); //? No XD
    // nnNetwork = new synaptic.Architect.Perceptron(5, 8, 8, 2); //? No
    // nnNetwork = new synaptic.Architect.Perceptron(5, 6, 6, 2); //? No
    // nnNetwork = new synaptic.Architect.Perceptron(5, 12, 12, 12, 3); //? Funcionaba
    nnNetwork = new synaptic.Architect.Perceptron(5, 16, 3); //? Funciona con las entradas no separadas.
  ...
  function recopilarDatosEntrenamiento() {
    if (!modoAuto) {
      ...
      // if (balaVerticalD) {
      //   console.log(`aire:${estadoEnAire} derecha:${estadoMovDerecha} izquierda: ${estadoMovIzquierda}`);
      //   datosEntrenamiento.push({
      ...
      if (balaD || balaVerticalD) {
        datosEntrenamiento.push({
          ...
          output: [estadoEnAire, estadoMovDerecha, estadoMovIzquierda],
        });
      }
  
      console.log(`
        ${desplazamientoBala},${velocidadBala},${desplazamientoBalaV},${velocidadBalaV},${posicionXBalaV},${estadoEnAire},${estadoMovDerecha},${estadoMovIzquierda}`);
  ...
  function datosDeEntrenamiento(param_entrada) {
    ...
    console.log(`${param_entrada[0]},${param_entrada[1]},${param_entrada[2]},${param_entrada[3]},${param_entrada[4]},${salidaRedondeada[0]},${salidaRedondeada[1]},${salidaRedondeada[2]}`);
    // console.log(
    //   "Salida",
    //   `En el Aire %: ${salidaRedondeada[0]},Mov Derecha %: ${salidaRedondeada[1]}, Mov Izquierda %: ${salidaRedondeada[2]} `
    // );
  ...
  function actualizarEstadoJugador() {
    ...
    estadoMovDerecha = jugador.body.velocity.x === 350 ? 1 : 0;
    estadoMovIzquierda = jugador.body.velocity.x === -350 ? 1: 0;
    // console.log(jugador.body.velocity.x);
  }
    ...
    if (modoAuto) { //? Modo Automatico
      ...
      if (jugadorD) {
        if (nnSalida[1] >= UMBRAL_MEDIO) { //+ Si la red decide "moverse a la derecha"
          ...
          // console.log(bala.position.x);
          // console.log("Velocidad Bala: ", velocidadBala);
          moverEjeXJugador(moverseDerecha, 200, 'r');
          // if ( balaVertical.position.x > 69.9999 && balaVertical.position.x <= 125.0 ) { //110
          //   moverEjeXJugador(moverseIzquierda, 200, 'l');
          // } else {
          //   moverEjeXJugador(moverseDerecha, 200, 'r');
          // }
        } else if (nnSalida[2] >= UMBRAL_MEDIO) { //+ Si la red decide "moverse a la izquierda"
          // console.log(balaVertical.position.x);
          // console.log(bala.position.x);
          // console.log("Velocidad Bala: ", velocidadBala);
          moverEjeXJugador(moverseIzquierda, 200, 'l');
          // if ( balaVertical.position.x < 69.9999 && balaVertical.position.x >= 15.0 ) {
          //   // moverEjeXJugador(moverseDerecha, 200, 'r');
          // } else {
          //   moverEjeXJugador(moverseIzquierda, 200, 'l');
          // }
  ...
  function disparoVertical() {
    velocidadBalaV = 1 * velocidadRandom(200, 300);
